...
def get_attendance(service):
    ...
    for course in total_session.keys():
        ...
        attendance_percentage = (attended_sessions / total_sessions_held) * 100 if total_sessions_held > 0 else 0
...
def get_max_attendance(service):
    ...
    events = service.events().list(
        calendarId=CAL_ID,
        timeMin=semester_class_start_date.isoformat() + 'Z',
        timeMax=semester_class_end_date.isoformat() + 'Z',singleEvents=True,orderBy='startTime'
    ).execute()
    ...
    for course in total_session.keys():
        ...
        max_attendance_percentage = (max_attended_sessions / total_possible_held) * 100 if total_possible_held > 0 else 0
        print(f"Course: {course}")
