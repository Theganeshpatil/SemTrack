...
from app.api import attendance_routes, auth_routes
...
@app.get("/")
def home():
    return {"SemTrack": "API is running"}

app.include_router(attendance_routes.router, tags=["Attendance"], prefix="/attendance")
...
#     service = build("calendar", "v3", credentials=create_credentials_from_jwt(jwt_token))
#     res = attendance_routes.get_attendance(service, semester_class_start_date = SEM_START_DATE, semester_class_end_date=SEM_END_DATE,cal_id=CAL_ID)
